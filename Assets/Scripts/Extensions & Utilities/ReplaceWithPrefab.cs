    ...
    private void OnGUI()
    {
        ...
        if (GUILayout.Button("Replace"))
        {
            ...
            for (var i = selection.Length - 1; i >= 0; --i)
            {
                ...
                // -- also set each newobject.transform of the children the same.
                for (int c = 0; c < newObject.transform.childCount; c++)
                {
                    Transform newObjectChild = newObject.transform.GetChild(c);
                    Transform selectedObjectChild = selected.transform.GetChild(c);

                    newObjectChild.transform.localPosition = selectedObjectChild.transform.localPosition;
                    newObjectChild.transform.localRotation = selectedObjectChild.transform.localRotation;
                    newObjectChild.transform.localScale = selectedObjectChild.transform.localScale;
                }
